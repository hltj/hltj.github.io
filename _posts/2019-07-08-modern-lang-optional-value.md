...
因此，现代编程语言基本都会避免使用传统空值，而采用更安全的方式来表达可选值，具体方式主要有两种：
  1. 受限的空值：Kotlin、Swift、Hack、C# 8+ 等。
...
采用受限空值的语言可能都与历史包袱有关：Kotlin 中有 `null` 因为 Java 中有，Hack 中有 `null` 因为 PHP 中有，Swift 中有 `nil` 因为 Objective-C 中有，C# 8+ 中有 `null` 是因为 C# 的历史版本中有且需要与 .Net 平台其他语言互操作。
...
这里特别值得一提的是，[C# 8 的选择](https://docs.microsoft.com/en-us/dotnet/csharp/tutorials/upgrade-to-nullable-references)给了大家一个启示：
**即便曾经做了错误的设计，仍然可以重回正轨**，就看语言的后续版本有没有壮士断腕的勇气。
...
### 区分可空与非空类型
Kotlin、Swift、Hack、C# 8+[^1] 都严格区分可空类型与非空类型，并且类型都默认非空，对于可空类型也都采用加注 `?` 的方式来表达（只是 Hack 放在类型名前，Kotlin、Swift 与 C# 放在类型名后）。我们看些具体的示例：

[^1]: C# 对于引用类型的严格可空性区分是 C# 8 引入的，而 C# 值类型自 C# 2.0 引入可空支持起一直就区分。
...
除了变量声明与赋值、函数返回值之外，这几门语言对函数传参、数学运算等各种表达式都会严格区分可空与非空类型。

### 可空性传播
在采用受限空值的编程语言中，无法直接访问可空类型对象的成员，需要使用特殊语法。在 Kotlin、Swift 与 C# 8+ 中使用 `?.` 语法，在 Hack 中使用 `?->` 语法。例如输出一个可空字符串的长度：
...
可空性就像病毒一样感染了整个调用链条，并且会继续传播下去。在 C# 8+ 中也是如此；在 Hack 中与此类似，只是使用 `?->` 语法。不过 Swift 的语法与它们不同，对于上述情况，后续链条中用 `.` 即可，但是最终结果仍然是可空值：
...
此外 Swift 与 C# 还支持可空对象的索引访问（`?[]`）语法，C# 8 还引入了空接合赋值操作符（`??=`{:style="white-space: nowrap;"}）。
...
### 由可空表达式获得非空值
这里只讨论安全获得非空值的方式。由可空表达式安全地获得非空值还需要提供一个默认值，这样就一定能够取得非空值：当表达式求值结果非空时取求值结果，否则取默认值。这在  Kotlin 中通过 Elvis 操作符（`?:`）来实现，在 Swift、Hack 与 C# 中通过空接合操作符（`??`）来实现。
...
实际上，可选值类型是 [Functor、Applicative、Monad](https://hltj.me/kotlin/2017/08/25/kotlin-functor-applicative-monad-cn.html)，上述 `Optional.map()` 相当于 Haskell 中 `Functor` 的 `fmap`/`<$>`。此外，常用的还有相当于 `Monad` 的 `>>=` 的函数，如 Java 的 `Optional.flatMap()`、Rust 的 `Option::and_then()`、OCaml 4.08+/F# 的 `Option.bind` 等。我们看一个 F# 的示例——对一个整数可选值求余：
...
> 上述代码只需将 `%` 运算符替换为 `mod` 并交换 `Option.bind` 两个参数的位置即可用于 OCaml 4.08+。
